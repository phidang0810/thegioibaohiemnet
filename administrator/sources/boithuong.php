...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=boithuong&act=man");
	
	$sql = "select * from #_news where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=boithuong&act=man");
	$item = $d->fetch_array();
}
...
function save_item(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=boithuong&act=man");
	...
	if($id){
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=boithuong&act=man");
	}else{
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=boithuong&act=man");
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=boithuong&act=man");
		...
		} redirect("index.php?com=boithuong&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=boithuong&act=man");
}
//===========================================================
function get_cats(){
	global $d, $items, $paging;
...
function get_cat(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=boithuong&act=man_cat");
	
	$sql = "select * from #_news_item where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=boithuong&act=man_cat");
	$item = $d->fetch_array();
}
...
function save_cat(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=boithuong&act=man_cat");
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	if($id){
		$data['ten_vi'] = $_POST['ten_vi'];
		$data['ten_en'] = $_POST['ten_en'];
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=boithuong&act=man_cat");
	}else{
	$data['ten_vi'] = $_POST['ten_vi'];
		$data['ten_en'] = $_POST['ten_en'];
		$data['tenkhongdau'] = changeTitle($_POST['ten_vi']);
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=boithuong&act=man_cat");
	}
}
...
function delete_cat(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=boithuong&act=man_cat");
	}else transfer("Không nhận được dữ liệu", "index.php?com=boithuong&act=man_cat");
}
?>